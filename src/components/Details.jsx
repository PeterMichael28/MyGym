...
const Details = ({ exerciseDetail }) => {
  ...
  return (
    <Stack gap="60px" sx={{ flexDirection: { lg: 'row' }, p: '20px', alignItems: 'center', mt:{lg:'-100px'} }}>
      <img src={gifUrl} alt={name} loading="lazy" className="detail-image" />
      <Stack sx={{ gap: { lg: '35px', xs: '20px' } }}>
        <Typography sx={{ fontSize: { lg: '54px', xs: '30px' } }} fontWeight={700} textTransform="capitalize">
          {name}
        </Typography>
        <Typography sx={{ fontSize: { lg: '22px', xs: '16px' } }} color="#4F4C4C">
        ...
        {extraDetail?.map((item) => (
          <Stack key={item.id} direction="row" gap="20px" alignItems="center">
